    ...
    <div v-else-if="!currentUser" class="min-h-screen flex items-center justify-center bg-[#F5F5F7] p-4">
        <div class="bg-white p-8 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] w-full max-w-md border border-gray-100 animate-pop hover-lift">
            ...
            <div v-if="authView === 'register'" class="space-y-4 animate-pop">
                <input v-model="authForm.email" type="email" placeholder="Email (@gmail.com or @organization.com)" class="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/50 input-smooth text-sm">
                <input v-model="authForm.username" type="text" placeholder="Choose Username" class="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/50 input-smooth text-sm">
                <input v-model="authForm.password" type="password" placeholder="Password" class="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/50 input-smooth text-sm">
                <input v-model="authForm.confirmPassword" type="password" placeholder="Confirm Password" class="w-full bg-gray-50 border border-gray-200 p-3 rounded-xl outline-none focus:ring-2 focus:ring-blue-500/50 input-smooth text-sm">
    ...
    <div v-else class="flex-grow flex flex-col animate-pop">
            ...
            <nav class="max-w-7xl mx-auto glass-nav-premium rounded-3xl shadow-[0_8px_32px_rgba(0,0,0,0.06)] text-gray-900 p-2 flex flex-wrap justify-between items-center transition-all duration-500 hover:shadow-[0_12px_48px_rgba(0,0,0,0.09)]">
                ...
                <div class="flex items-center p-1.5 bg-gray-100/60 rounded-2xl border border-gray-200/50 overflow-x-auto no-scrollbar">
                    <button @click="activeTab = 'dashboard'" :class="tabClass('dashboard')" class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm transition-all duration-300 relative group outline-none">
                        <i class="fas fa-chart-pie" :class="activeTab === 'dashboard' ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600'"></i><span class="hidden md:inline">Dashboard</span>
                    </button>
                    <button @click="activeTab = 'inventory'" :class="tabClass('inventory')" class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm transition-all duration-300 relative group outline-none">
                        <i class="fas fa-boxes" :class="activeTab === 'inventory' ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600'"></i><span class="hidden md:inline">Inventory</span>
                    </button>
                    <button @click="activeTab = 'add'" :class="tabClass('add')" class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm transition-all duration-300 relative group outline-none">
                        <i class="fas fa-plus-circle" :class="activeTab === 'add' ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600'"></i><span class="hidden md:inline">Add Device</span>
                    </button>
                    <button @click="activeTab = 'profile'" :class="tabClass('profile')" class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm transition-all duration-300 relative group outline-none">
                        <i class="fas fa-user-circle" :class="activeTab === 'profile' ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600'"></i><span class="hidden md:inline">Profile</span>
                    </button>
                    ...
                    <button v-if="['admin', 'moderator'].includes(currentUser.role)" @click="activeTab = 'adminPanel'" :class="tabClass('adminPanel')" class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm transition-all duration-300 relative group outline-none">
                        <i class="fas fa-shield-alt" :class="activeTab === 'adminPanel' ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600'"></i><span class="hidden md:inline">Admin</span>
                    ...
                    <button v-if="['admin', 'moderator'].includes(currentUser.role)" @click="activeTab = 'bulk'" :class="tabClass('bulk')" class="flex items-center gap-2 px-4 py-2 rounded-xl text-sm transition-all duration-300 relative group outline-none">
                        <i class="fas fa-layer-group" :class="activeTab === 'bulk' ? 'text-blue-500' : 'text-gray-400 group-hover:text-gray-600'"></i><span class="hidden md:inline">Bulk</span>
                    </button>
                </div>
        ...
        <main class="flex-grow p-4 md:p-6 max-w-7xl mx-auto w-full relative">
            ...
            <div v-if="activeTab === 'dashboard'" class="space-y-8 animate-pop">
                ...
                <div class="bg-white p-8 rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 hover-lift">
                    ...
                    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                        <div v-for="st in ['Pending', 'Quick Repair Stage', 'Complex Stage', 'Ready', 'Irreparable']" :key="st" class="relative">
            ...
            <div v-if="activeTab === 'inventory'" class="space-y-6 animate-pop">
                ...
                <div v-for="asset in filteredAssets" :key="asset.tag" class="bg-white rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 overflow-hidden mb-4 hover-lift">
                    ...
                    <div class="p-5 grid md:grid-cols-2 gap-8">
                        <div class="text-sm">
                            ...
                            <div class="max-h-48 overflow-y-auto space-y-3 pr-2 smooth-scroll">
                                <div v-for="rep in asset.repairs" class="bg-gray-50/50 p-3 rounded-2xl border border-gray-100 shadow-sm relative overflow-hidden transition-all hover:bg-gray-100">
                        ...
                        <div class="bg-white p-5 rounded-3xl border border-gray-100 shadow-[0_2px_10px_rgb(0,0,0,0.02)] space-y-3">
            ...
            <div v-if="activeTab === 'profile'" class="max-w-md mx-auto space-y-6 animate-pop">
                ...
                <div class="bg-white p-8 rounded-3xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 hover-lift">
                    ...
                    <div class="space-y-4">
                        ...
                        <div><label class="text-[11px] font-bold text-gray-500 uppercase tracking-wider ml-1">New Email Address</label><input v-model="profileUpdate.newEmail" type="email" placeholder="new@domain.com" class="w-full bg-gray-50 border border-gray-200 p-3.5 rounded-xl mt-1.5 outline-none focus:ring-2 focus:ring-blue-500/50 input-smooth"></div>
                        <div><label class="text-[11px] font-bold text-gray-500 uppercase tracking-wider ml-1">Confirm New Email</label><input v-model="profileUpdate.confirmEmail" type="email" placeholder="confirm@domain.com" class="w-full bg-gray-50 border border-gray-200 p-3.5 rounded-xl mt-1.5 outline-none focus:ring-2 focus:ring-blue-500/50 input-smooth"></div>
...
<script>
    ...
    createApp({
        ...
        computed: {
            ...
            // Core Inventory & Pending Assets Filters
            ...
            inRepairCount() { return this.inventoryAssets.filter(a => ['Received for Repair', 'Pending', 'Quick Repair Stage', 'Complex Stage'].includes(a.status)).length; },
            statusCounts() { const c = {}; this.inventoryAssets.forEach(a => { c[a.status] = (c[a.status] || 0) + 1; }); return c; },
            
            allTimeRepairedCount() { 
                return this.inventoryAssets.reduce((sum, asset) => sum + parseInt(asset.repairCount || 0, 10), 0); 
            },
        ...
        methods: {
            ...
            async updateHeartbeat() {
                ...
                await fetch(`api.php?action=ping&username=${this.currentUser.username}`);
                const res = await fetch('api.php?action=load');
                const data = await res.json();
            ...
            statusBg(st) { const map = { 'Pending': 'bg-yellow-400', 'Quick Repair Stage': 'bg-blue-400', 'Complex Stage': 'bg-orange-400', 'Ready': 'bg-green-400', 'Irreparable': 'bg-red-400' }; return map[st] || 'bg-gray-300'; },
            statusColor(s) { return s === 'Ready' ? 'bg-green-50 text-green-700 border-green-200' : (s === 'Irreparable' ? 'bg-red-50 text-red-700 border-red-200' : 'bg-blue-50 text-blue-700 border-blue-200'); },
            ...
            isOnline(user) { return user.lastSeen && (Date.now() - user.lastSeen) < 15000; },
            ...
            changeEmail() {
                ...
                if (!newEmail || !this.isValidEmail(newEmail)) return alert("Please enter a valid email address.");
            ...
            register() { 
                ...
                const emailRegex = /^[\w-\.]+@(gmail\.com|organization\.com)$/i;
                if(!emailRegex.test(this.authForm.email)) return this.showMessage("Email must end with @gmail.com or @organization.com");
            ...
            addSingleDevice() { 
                ...
                // NEW: Users go to pending state. Admins/Mods go direct to repair.
                const initialStatus = this.currentUser.role === 'user' ? 'Pending Approval' : 'Received for Repair';
                const newAsset = { ...this.singleForm, status: initialStatus, repairs: [], repairCount: 0 };
            ...
            // NEW: Admin Actions for Assets
            approveAsset(tag) {
                const a = this.assets.find(x => x.tag === tag);
                if(a) {
                    a.status = 'Received for Repair';
                    this.saveDbAsset(a);
            ...
            addRepair(tag) { 
                const a = this.assets.find(x => x.tag === tag); const d = this.draftRepairs[tag]; 
                if(a && d && d.problem) { 
                    if(!a.repairs) a.repairs = []; 
                    a.repairs.push({...d, date: new Date().toLocaleDateString()}); 
                    this.draftRepairs[tag] = {problem:'', repSerial:'', cost:0}; 
                    if(this.saveDbAsset) this.saveDbAsset(a);
                    else if(this.saveDbAssets) this.saveDbAssets();
                } 
            },
            getDraft(tag) { if(!this.draftRepairs[tag]) this.draftRepairs[tag] = {problem:'', repSerial:'', cost:0}; return this.draftRepairs[tag]; },
                    ...
                    setTimeout(() => {
                        rows.forEach((row, i) => { 
                            if(i > 0 && row.trim()) { 
                                ...
                                const newAsset = { tag, type, brand, model, serial, purchaseDate, status: 'Received for Repair', repairs: [], repairCount: 0 };
